...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recv_data)
{
    ...
    TrainerSpell const* TrainerSpell = cSpells ? cSpells->Find(spellId) : NULL;

    // Not found, try find in npc_trainer_template
    if (!TrainerSpell && tSpells)
        TrainerSpell = tSpells->Find(spellId);
    ...
    uint32 reqLevel = 0;
    if (!_player->IsSpellFitByClassAndRace(TrainerSpell->learnedSpell, &reqLevel))
        trainState = 1;

    reqLevel = TrainerSpell->isProvidedReqLevel ? TrainerSpell->reqLevel : std::max(reqLevel, TrainerSpell->reqLevel);
    if (_player->GetTrainerSpellState(TrainerSpell, reqLevel) != TRAINER_SPELL_GREEN)
        trainState = 1;

    // apply reputation discount
    uint32 nSpellCost = uint32(floor(TrainerSpell->spellCost * _player->GetReputationPriceDiscount(unit)));
